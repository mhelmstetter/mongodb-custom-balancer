...
/**
 * Selects the smallest chunk (by document count or size) from available chunks.
 * This can help avoid ChunkTooBig errors and reduce migration impact.
 *
 ...
 */
public class SmallestChunkSelector implements ChunkSelectionStrategy {

    private static final Logger logger = LoggerFactory.getLogger(SmallestChunkSelector.class);
    private final ShardClient shardClient;

    public SmallestChunkSelector(ShardClient shardClient) {
    ...
    public Megachunk selectChunk(ShardMetrics sourceShardMetrics, List<Megachunk> availableChunks) {
        ...
        logger.debug("SmallestChunkSelector: selecting random chunk (size calculation not yet implemented)");
